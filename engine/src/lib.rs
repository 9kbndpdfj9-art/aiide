...
impl Engine {
    ...
    pub fn process_file_change(&mut self, path: &str, content: &str) -> Result<Vec<String>, Box<dyn std::error::Error>> {
        ...
        for r in &new_refs {
            if !old_refs.contains(r) && !affected.contains(r) {
                affected.push(r.clone());
            }
        }
        for r in &old_refs {
            if !new_refs.contains(r) && !affected.contains(r) {
                affected.push(r.clone());
            }
        }
        ...
        self.observer.log_event("file_change", &serde_json::json!({
            "path": path,
            "added_refs": new_refs.iter().filter(|r| !old_refs.contains(r)).cloned().collect::<Vec<_>>(),
            "removed_refs": old_refs.iter().filter(|r| !new_refs.contains(r)).cloned().collect::<Vec<_>>(),
