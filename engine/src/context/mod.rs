...
impl ContextManager {
    ...
    pub fn get_usage_ratio(&self) -> f64 {
        if self.max_tokens == 0 { return 1.0; }
        self.current_tokens as f64 / self.max_tokens as f64
    }

    pub fn should_compress(&self) -> Option<CompressionLevel> {
        let ratio = self.get_usage_ratio();
        if ratio > 0.95 {
            Some(CompressionLevel::L3)
        } else if ratio > 0.85 {
            Some(CompressionLevel::L2)
        } else if ratio > 0.70 {
            Some(CompressionLevel::L1)
        } else {
            None
        }
    }
    ...
    pub fn compress_to_level(&mut self, target_level: CompressionLevel) -> Vec<String> {
        ...
        for entry in entries {
            if (entry.level as u8) >= (target_level as u8) {
                continue;
            }

            let old_tokens = entry.token_estimate;
            let (new_content, new_tokens) = match target_level {
                CompressionLevel::L1 => Self::compress_l1(&entry.content),
                CompressionLevel::L2 => Self::compress_l2(&entry.content),
                CompressionLevel::L3 => Self::compress_l3(&entry.content),
                CompressionLevel::L0 => (entry.content.clone(), entry.token_estimate),
            };

            let ratio = if old_tokens > 0 { new_tokens as f64 / old_tokens as f64 } else { 1.0 };

            entry.summary = Some(new_content);
            ...
            compressed.push(format!("{}: {} -> {} tokens ({:.0}%)",
                entry.id.chars().take(8).collect::<String>(),
                old_tokens, new_tokens, ratio * 100.0
            ));
        }
    ...
    pub fn auto_compress(&mut self) -> Option<CompressionLevel> {
        let level = self.should_compress()?;
        self.compress_to_level(level);
        Some(level)
    }
    ...
    pub fn get_active_context(&self) -> String {
        ...
        for entry in entries {
            let content = match entry.level {
                CompressionLevel::L0 => &entry.content,
                _ => entry.summary.as_ref().unwrap_or(&entry.content),
            };
    ...
    fn compress_l1(content: &str) -> (String, usize) {
        ...
        for line in cleaned.lines() {
            ...
            let is_signature = trimmed.starts_with("function ")
        ...
        let tokens = Self::estimate_tokens(&result);
        (result, tokens)
    }
    ...
    fn compress_l3(content: &str) -> (String, usize) {
        let keywords = Self::extract_keywords(content);
        let result = format!("// [L3 Index - keywords: {}]", keywords.join(", "));
        let tokens = Self::estimate_tokens(&result);
        (result, tokens)
    }
    ...
    fn extract_keywords(content: &str) -> Vec<String> {
        let stop_words = ["the", "a", "an", "is", "are", "was", "were", "be", "been", "being",
            ...
            "if", "else", "return", "const", "let", "var", "function", "class", "this",
            "new", "true", "false", "null", "undefined", "void", "static", "public",
            "private", "protected", "override", "final", "abstract", "impl"];
...
mod tests {
    ...
    #[test]
    fn test_context_add_and_get() {
        let mut cm = ContextManager::new();
        let id = cm.add_entry("fn main() { println!(\"hello\"); }", 1.0, vec!["rust".to_string()]);
        assert!(cm.get_entry(&id).is_some());
        assert_eq!(cm.entry_count(), 1);
    }

    #[test]
    fn test_context_dedup() {
        let mut cm = ContextManager::new();
        let id1 = cm.add_entry("same content", 1.0, vec![]);
        let id2 = cm.add_entry("same content", 0.5, vec![]);
        assert_eq!(id1, id2);
        assert_eq!(cm.entry_count(), 1);
    }

    #[test]
    fn test_compress_l1_strips_comments() {
        let code = "// this is a comment\nfn main() {\n  println!(\"hello\");\n}\n/* block\n   comment */\nfn other() {}";
        let (result, _) = ContextManager::compress_l1(code);
