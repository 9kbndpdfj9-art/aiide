...
impl<K: Clone + Hash + Eq, V: Clone> LruCache<K, V> {
    ...
    pub fn put(&mut self, key: K, value: V) -> Option<V> {
        if self.entries.contains_key(&key) {
            self.move_to_front(&key);
            let old = std::mem::replace(&mut self.entries.get_mut(&key).unwrap().value, value);
            return Some(old);
        }
    ...
    pub fn peek_lru(&self) -> Option<&V> {
        self.tail.as_ref().and_then(|k| self.entries.get(k).map(|e| &e.value))
    }
    ...
    fn unlink_from_list(&mut self, prev: Option<K>, next: Option<K>, key: &K) {
        ...
        if let Some(ref next_key) = next {
            ...
            self.tail = self.entries.get(key).and_then(|e| e.prev.clone());
        }
    }
    ...
    fn evict_lru(&mut self) {
        let tail_key = match self.tail.clone() {
            Some(k) => k,
            None => return,
        };

        let entry = match self.entries.remove(&tail_key) {
            Some(e) => e,
            None => return,
        };
