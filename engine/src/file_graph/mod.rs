...
impl FileGraph {
    ...
    pub fn get_impact_set(&self, path: &str) -> Vec<String> {
        ...
        if let Some(&idx) = self.path_index.get(path) {
            let distances: HashMap<NodeIndex, f64> = dijkstra(&self.graph, idx, None, |e| *e.weight());
            for (&node_idx, &dist) in &distances {
                if node_idx != idx && dist <= 2.0 {
                    if let Some(node) = self.graph.node_weight(node_idx) {
                        impact.push(node.path.clone());
                    }
                }
            }
        }
    ...
    pub fn get_impact_set_with_depth(&self, path: &str, max_depth: f64) -> Vec<(String, f64)> {
        let mut impact = Vec::new();
        if let Some(&idx) = self.path_index.get(path) {
            let distances: HashMap<NodeIndex, f64> = dijkstra(&self.graph, idx, None, |e| *e.weight());
            for (&node_idx, &dist) in &distances {
                if node_idx != idx && dist <= max_depth {
                    if let Some(node) = self.graph.node_weight(node_idx) {
                        impact.push((node.path.clone(), dist));
                    }
                }
            }
        }
        impact.sort_by(|a, b| a.1.partial_cmp(&b.1).unwrap_or(std::cmp::Ordering::Equal));
        impact
    }

    pub fn get_import_cycles(&self) -> Vec<Vec<String>> {
        ...
        for &start in &nodes {
            ...
            while let Some((current, path)) = stack.pop() {
                ...
                for neighbor in self.graph.neighbors_directed(current, petgraph::Direction::Outgoing) {
                    ...
                    } else if !visited.contains_key(&neighbor) {
    ...
    fn rebuild_edges_for(&mut self, path: &str, imports: &[String]) {
        let idx = match self.path_index.get(path) {
            Some(&i) => i,
            None => return,
        };

        let edges_to_remove: Vec<(NodeIndex, NodeIndex)> = self.graph
        ...
        for import_path in imports {
            if let Some(&target_idx) = self.path_index.get(import_path) {
                if self.graph.find_edge(idx, target_idx).is_none() {
                    self.graph.add_edge(idx, target_idx, 1.0);
                }
            }
        }
    }
    ...
    fn rebuild_incoming_edges_for(&mut self, path: &str) {
        let target_idx = match self.path_index.get(path) {
            Some(&i) => i,
            None => return,
        };

        let paths_that_import_this: Vec<NodeIndex> = self.path_index.iter()
            .filter(|(p, _)| *p != path)
            .filter_map(|(other_path, &other_idx)| {
                let node = &self.graph[other_idx];
                if node.imports.contains(&path.to_string()) {
                    Some(other_idx)
                } else {
                    None
                }
        ...
        for source_idx in paths_that_import_this {
    ...
    fn strip_comments_from_line(line: &str) -> &str {
    ...
    fn extract_imports(content: &str) -> Vec<String> {
        ...
        for line in content.lines() {
            ...
            let cleaned = Self::strip_comments_from_line(trimmed);
            let cleaned = cleaned.trim();

            for (re, is_js) in IMPORT_PATTERNS.iter() {
                for cap in re.captures_iter(cleaned) {
                    if let Some(m) = cap.get(1) {
                        let mut import_path = m.as_str().to_string();
                        if *is_js && !import_path.starts_with('.') {
                            continue;
                        }
                        if *is_js {
    ...
    fn extract_exports(content: &str) -> Vec<String> {
        ...
        for line in content.lines() {
            ...
            for re in EXPORT_PATTERNS.iter() {
                    ...
                    if let Some(m) = cap.get(1) {
                        for name in m.as_str().split(',') {
                            let name = name.trim()
                                ...
                                .trim();
                            if !name.is_empty() && !exports.contains(&name.to_string()) {
...
mod tests {
    ...
    fn test_file_graph_basic() {
        ...
        fg.update_file("main.js", "import { foo } from './utils.js'");
        fg.update_file("utils.js", "export function foo() {}");

        let refs = fg.get_references("main.js");
        assert!(refs.contains(&"utils.js".to_string()));

        let ref_by = fg.get_referenced_by("utils.js");
        assert!(ref_by.contains(&"main.js".to_string()));
    }

    #[test]
    fn test_file_graph_imports_strip_comments() {
        let code = "// import { old } from './old.js'\nimport { foo } from './utils.js'";
        let mut fg = FileGraph::new();
        fg.update_file("main.js", code);
        fg.update_file("utils.js", "export function foo() {}");

        let refs = fg.get_references("main.js");
        assert!(!refs.contains(&"old.js".to_string()));
        assert!(refs.contains(&"utils.js".to_string()));
    }
}
